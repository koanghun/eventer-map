...
> [!WARNING]
> **이 디렉토리의 마이그레이션 스크립트들은 더 이상 사용되지 않습니다.**
> 
> 현재 프로젝트는 **Alembic**을 사용하여 마이그레이션을 관리합니다.  
> 새로운 마이그레이션은 [`../migrations/`](../migrations/) 디렉토리를 사용하세요.
> 
> 이 디렉토리는 **히스토리 참고용**으로만 보관됩니다.

---

이 디렉토리는 Alembic 도입 이전에 사용되던 수동 SQLite 마이그레이션 스크립트를 포함합니다.
...
### 2025_12_15_remove_name_fields.py
- **날짜**: 2025-12-15
- **설명**: Performer 및 Place 테이블에서 deprecated `name` 컬럼 제거
- **실행 여부**: 
  - ⬜ 로컬 개발 환경
  - ⬜ Synology NAS 프로덕션 환경
- **주의**: 이 마이그레이션은 `name` 컬럼을 영구적으로 삭제합니다. 백업 권장!
