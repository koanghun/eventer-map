...
def create_performer(
    ...
    if status_code == 'created':
        return performer_obj
    
    # 병합되거나 이미 존재하는 경우 200 OK와 함께 상태 전송
    message = "기존 출연자에 별칭이 추가되었습니다" if status_code == 'merged' else "이미 동일한 출연자가 존재합니다"
    ...
    return JSONResponse(
        ...
        content={
            ...
            "performer": {
                "id": performer_obj.id,
                "canonical_name": performer_obj.canonical_name,
                "normalized_name": performer_obj.normalized_name,
                "aliases": json_to_aliases(performer_obj.aliases),
            }
        }
    )
