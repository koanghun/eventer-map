...
- **방식:** 자체 JWT 발급 (Access Token + Refresh Token)
- **제공자:** 
  1. **자체 이메일/비밀번호 로그인 (로컬 인증)**
  2. **Google 소셜 로그인 및 기존 계정 연동**
- **보안 처리:** 
  - Access Token: JSON Body로 전달받아 클라이언트 메모리에서 관리 (CSRF 방어)
  - Refresh Token: `HttpOnly` 쿠키를 통해 브라우저에 저장 (XSS 방어)
